...
def save_mask(pred_masks, save_base_path, category_list, video_name_list):
    ...
    category, video_name = category_list, video_name_list
    mask_save_path = os.path.join(save_base_path, category, video_name)
    if not os.path.exists(mask_save_path):
        os.makedirs(mask_save_path, exist_ok=True)
    one_video_masks = pred_masks[0] # [5, 1, 224, 224]
...
def save_img(pred_masks, save_base_path, category_list, video_name_list):
    # pred_mask: [bs*5, 1, 224, 224]
    # print(f"=> {len(video_name_list)} videos in this batch")
    ...
    pred_masks = pred_masks.squeeze(1) #[5,3,224,224]
    pred_masks = pred_masks.permute(0,2,3,1)
    # pred_masks = pred_masks.transpose(0,1,3,2)
    # pred_masks = (torch.sigmoid(pred_masks) > 0.5).int()
    ...
    if not os.path.exists(mask_save_path):
        os.makedirs(mask_save_path, exist_ok=True)
    # one_video_masks = pred_masks[0] # [5, 1, 224, 224]
    for video_id in range(len(pred_masks)):
        one_mask = pred_masks[video_id]
        output_name = "%s_%d.png"%(video_name, video_id)
